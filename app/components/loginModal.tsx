...
function Login({login, unShowModal}: {login: (username: string, password: string) => void, unShowModal: () => void}) {
    ...
    function handleUsernameChange(input: { target: { value: SetStateAction<string>; }; }) {
        setUsername(input.target.value);
    }

    function handlePasswordChange(input: { target: { value: SetStateAction<string>; }; }) {
    ...
    return <div className='fixed m-auto inset-0 flex flex-col justify-center bg-gray-800 z-50 space-y-5 w-60 h-60 p-5 rounded'>
        ...
        <input className='rounded border-gray-500 bg-inherit pt-5 text-white' placeholder="Username" value={username} onChange={handleUsernameChange} id='username' />
        <input className='rounded border-gray-500 bg-inherit pb-5 text-white' type="password" placeholder="Password" value={password} onChange={handlePasswordChange} id='password'/>
